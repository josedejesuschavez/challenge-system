...
from src.Application.Service.Pits.add_team_member import AddTeamMember
...
blueprint = Blueprint('post_teammember', __name__)

@blueprint.route('/teammember', methods=['POST'])
@cross_origin()
@jwt_required()
def post_team_member():
    ...
    application_service = AddTeamMember(id=str(uuid.uuid4()),
                                        team_member=team_member,
                                        repository=ElasticSearchTeamMemberRepository())
    application_service.execute()
    return jsonify({'team_members': 'asfsaf'})
